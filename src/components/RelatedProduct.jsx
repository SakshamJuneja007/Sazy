...
const LatestCollection = () => {
    ...
    const latestProducts = products ? products.slice(0, productLimit) : [];

    return (
        <div className="content-wrapper">
            {/* Section Title */}
            <div className="text-3xl text-center py-8">
                <Tittle text1="RELATED " text2="RELATED " />
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-y-6">
                {latestProducts.length > 0 ? (
                    latestProducts.map((product) => (
                        <div key={product._id} className="p-4 transition-transform hover:shadow-lg">
